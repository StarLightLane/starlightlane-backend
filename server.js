...
async function sendEmail(data) {
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        ...
        <tr>
          <td style="padding: 8px 0; color: #6b7280; font-size: 14px;">What brings you here?</td>
          <td style="padding: 8px 0; color: #111827; font-size: 14px;"><strong>${data.reason}</strong></td>
        </tr>
        <tr>
          <td style="padding: 8px 0; color: #6b7280; font-size: 14px; vertical-align: top;">Anything else?</td>
          <td style="padding: 8px 0; color: #111827; font-size: 14px;">${data.additionalInfo || "—"}</td>
        </tr>
      </table>
    </div>
  `;
  ...
  await transporter.sendMail({
    ...
    replyTo: data.email,
    subject: `New Contact: ${data.firstName} ${data.lastName} — ${data.reason}`,
    html,
  });
}
...
const VALID_REASONS = [
  "Lower my processing rate",
  "Recover a holdback or reserve",
  "Add AI to my existing setup",
  "All of the above",
  "Something else",
];

function validate(body) {
  const { firstName, lastName, email, monthlyVolume, reason } = body;
  ...
  if (!reason || !VALID_REASONS.includes(reason))
    return "Please select what brings you here.";
  return null;
}
